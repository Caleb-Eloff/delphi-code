...
uses
  ...
  Dialogs, SmartSwitch_U, StdCtrls, ComCtrls, Spin, ExtCtrls, Math;
...
var
  sSwitchID, sDevice: String;
  iPowerUsage, iPos: Integer;
...
begin
  ...
  sSwitchID := cmbQ3_2_1.Text;
  sDevice := lstQ3_2_1.Items[lstQ3_2_1.ItemIndex];
  iPos := Pos('#', sDevice);
  iPowerUsage := StrToInt(Copy(sDevice, iPos + 1, Length(sDevice)));
  Delete(sDevice, iPos, Length(sDevice));

  objSmartSwitch := TSmartSwitch.create(sSwitchID, sDevice, iPowerUsage);
...
begin
  ...
  objSmartSwitch.setSwitchStatus(bSwitch);
...
begin
  ...
  redQ3.Lines.Add('Energy used is: ' +
    FloatToStr(RoundTo(objSmartSwitch.energyUsed(iHours), -2)) + ' kWh');
